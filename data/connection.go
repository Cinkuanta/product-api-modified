...
func (c *PostgresSQL) AuthUser(username string, password string) (model.User, error) {
	...
	err := c.db.Select(&us,fmt.Sprintf("SELECT id, username FROM users WHERE username = '%s' AND password = crypt('%s',password);",username, password),)
	if err != nil {
	...
	if len(us) < 1 {
		...
		return model.User{}, errors.New(fmt.Sprintf("SELECT id, username FROM users WHERE username = '%s' AND password = '%s';",username, password))
	}
